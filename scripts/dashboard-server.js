...
app.post("/api/generate", async (req, res) => {
  ...
  const prompt = `
    ...
    - 글자수는 4000자에서 5000자 사이로 작성한다. (중요)
    - 본문에 반드시 데이터를 표(테이블) 형태로 포함한다. 비교표, 요약표, 조건표 등 키워드에 적합한 표를 최소 1개 이상 삽입한다. (중요)
...
${affiliateLinks.some(l => l.url && l.url.includes('coupang.com')) ? `    [이미지 안내]
    ...
    [목차(TOC) 구성 - 필수]
    - 글의 도입부(Frontmatter 바로 다음)에 다음 형식으로 목차를 반드시 포함한다:
    ...
    - 각 H2(##) 소제목에는 반드시 HTML 앵커 태그를 앞에 넣는다: <a id="anchor1"></a>## 소제목
    - 목차 항목은 본문의 H2 소제목과 정확히 일치해야 한다.
    - 목차를 누르면 해당 소제목으로 이동할 수 있어야 한다.
...
app.post("/api/publish", async (req, res) => {
  ...
  try {
    ...
    content = content.replace(/pubDatetime:\s*.*/, `pubDatetime: ${now}`);
    fs.writeFileSync(filePath, content, { encoding: "utf8" });
...
app.post("/api/bulk-generate", (req, res) => {
  ...
  (async () => {
    ...
    - 개인의 경험이나 의견을 반드시 포함하여 작성한다. 글자수 4000~5000자.
    - 본문에 반드시 데이터를 표(테이블) 형태로 포함한다. 비교표, 요약표, 조건표 등 키워드에 적합한 표를 최소 1개 이상 삽입한다.
